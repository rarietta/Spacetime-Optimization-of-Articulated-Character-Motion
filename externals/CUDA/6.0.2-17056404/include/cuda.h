...
/**
 * CUDA Managed Mem Flags
 */
typedef enum CUmanaged_flags_enum {
    CU_MANAGED_MEM_ATTACH_GLOBAL = 0x1, /**< Managed memory can be accessed by all streams on the device*/
    CU_MANAGED_MEM_ATTACH_HOST   = 0x2, /**< Managed memory cannot be accessed by any stream on the device */
    CU_MANAGED_MEM_ATTACH_SINGLE = 0x4  /**< Managed memory can only be accessed by a single stream on the device */
} CUmanaged_flags;
...
typedef enum CUjit_target_enum
{
    ...
    CU_TARGET_COMPUTE_50,       /**< Compute device class 5.0 */
    CU_MAX_JIT_TARGET
} CUjit_target;
...
CUresult CUDAAPI cuCompilePtx(void** cubin, size_t *cubinSize, const void *image, unsigned int numOptions, CUjit_option *options, void **optionValues);

CUresult CUDAAPI cuBinaryFree(void* cubin);
 ...
 * \p *dptr a pointer to the allocated memory. The allocated memory is suitably
 * aligned for any kind of variable. The memory is not cleared. If \p bytesize
 * is 0, ::cuMemAllocManaged() returns ::CUDA_ERROR_INVALID_VALUE.
 *
 * \param dptr     - Returned device pointer
 * \param bytesize - Requested allocation size in bytes
 * \param flags    - Must be one of CU_MANAGED_MEM_ATTACH_GLOBAL or CU_MANAGED_MEM_ATTACH_HOST
 *
 * \return
 * ::CUDA_SUCCESS,
 * ::CUDA_ERROR_DEINITIALIZED,
...
/**
 * \brief Attach managed data to a stream
 *
 * TODO : Add description
 *
 * \param hStream - Stream in which to enqueue the attach operation
 * \param dptr    - Pointer to managed data
 * \param length  - Length of managed data (must be zero)
 * \param flags   - Must be one of CUmanaged_flags
 *
 * \return
 * ::CUDA_SUCCESS,
 * ::CUDA_ERROR_DEINITIALIZED,
 ...
 * ::cuStreamDestroy
 */
CUresult CUDAAPI cuStreamAttachManagedMem(CUstream hStream, CUdeviceptr dptr, size_t length, unsigned int flags);
 ...
 *
 * Returns in \p occupancy the value of the maximum active blocks per
 * streaming multiprocessor.
 *
 * \param numBlocks - Returned occupancy
 * \param func  - kernel for which occupancy is calulated
 * \param numDynamicSmemBytes - dynamic shared memory used in bytes, provided
 *                              by user
 *
 * \return
 * ::CUDA_SUCCESS,
 ...
 *
 */
CUresult CUDAAPI cuOccMaxActiveBlocksPerMultiprocessor(int *numBlocks, CUfunction func, size_t numDynamicSmemBytes);
