		...
		if (validParticleRange > 0)
		{
			for (PxU32 w = 0; w <= (validParticleRange-1) >> 5; w++)
			{
				for (PxU32 b = validParticleBitmap[w]; b; b &= b-1)
				{
					...
					validPosBuf.pushBack(posBuffer[(int)index]);
					if(bVelBuff)
						validVelBuf.pushBack(velBuffer[(int)index]);
					if(perParticleRestOffset)
						validRestBuf.pushBack(restBuffer[(int)index]);
					if(bFlagBuff)
						validFlagBuf.pushBack(flagBuffer[(int)index]);
				}			
			} 
