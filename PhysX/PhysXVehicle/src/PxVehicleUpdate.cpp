...
PxVehicleDrive4W* vehDrive4W, PxVehicleWheelQueryResult* vehWheelQueryResults, PxVehicleConcurrentUpdateData* vehConcurrentUpdates)
{
	...
	else
	{
		...
		//a = (v - v0)/dt
		vehicleConcurrentUpdates.linearMomentumChange = (carChassisLinVel-carChassisLinVelOrig)*inverseTimestep;;
		vehicleConcurrentUpdates.angularMomentumChange = (carChassisAngVel-carChassisAngVelOrig)*inverseTimestep;;
	}
...
(const PxVehicleConcurrentUpdateData* vehicleConcurentUpdates, const PxU32 numVehicles, PxVehicleWheels** vehicles)
{
	PX_CHECK_AND_RETURN(vehicleConcurentUpdates, "vehicleConcurentUpdates must be non-null.");
	...
	for(PxU32 i=0;i<numVehicles;i++)
	{
		...
		for(PxU32 j=0;j<vehWheels->mWheelsSimData.mNbActiveWheels;j++)
		{
			...
			PX_CHECK_AND_RETURN(vehicleConcurentUpdates[i].concurrentWheelUpdates && vehicleConcurentUpdates[i].nbConcurrentWheelUpdates >= vehWheels->mWheelsSimData.getNbWheels(),
				"vehicleConcurentUpdates is illegally configured with either null pointers or insufficient memory for successful concurrent vehicle updates.");
		}
	}
#endif
	...
	for(PxU32 i=0;i<numVehicles;i++)
	{
		...
		//set sequentially.
		const PxVehicleConcurrentUpdateData& vehicleConcurrentUpdate = vehicleConcurentUpdates[i];
